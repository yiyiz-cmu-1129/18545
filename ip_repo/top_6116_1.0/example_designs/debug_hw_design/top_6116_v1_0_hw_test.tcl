# Runtime Tcl commands to interact with - top_6116_v1_0

# Sourcing design address info tcl
set bd_path [get_property DIRECTORY [current_project]]/[current_project].srcs/[current_fileset]/bd
source ${bd_path}/top_6116_v1_0_include.tcl
...
# Test: to_bram
# Create a write transaction at to_bram_addr address
create_hw_axi_txn w_to_bram_addr [get_hw_axis $jtag_axi_master] -type write -address $to_bram_addr -data $wdata_1
# Create a read transaction at to_bram_addr address
create_hw_axi_txn r_to_bram_addr [get_hw_axis $jtag_axi_master] -type read -address $to_bram_addr
# Initiate transactions
run_hw_axi r_to_bram_addr
run_hw_axi w_to_bram_addr
run_hw_axi r_to_bram_addr
set rdata_tmp [get_property DATA [get_hw_axi_txn r_to_bram_addr]]
# Compare read data
if { $rdata_tmp == $wdata_1 } {
	puts "Data comparison test pass for - to_bram"
} else {
	puts "Data comparison test fail for - to_bram, expected-$wdata_1 actual-$rdata_tmp"
	inc ec
}
...
# Test: to_system
set wdata_to_system [get_init_data 0]
create_hw_axi_txn w_to_system_addr [get_hw_axis $jtag_axi_master] -type write -address $axi_gpio_out_addr -data ${wdata_to_system}
create_hw_axi_txn r_to_system_addr [get_hw_axis $jtag_axi_master] -type read -address $axi_gpio_in_addr 
# Initiate transactions
run_hw_axi r_to_system_addr
run_hw_axi w_to_system_addr
run_hw_axi r_to_system_addr
set rdata_tmp [get_property DATA [get_hw_axi_txn r_to_system_addr]]
set DE [ get_done_and_error_bit $rdata_tmp 1 0 ]
# Compare read data
if { $DE == 01 } {
	puts "Data comparison test pass for - to_system"
} else {
	puts "Data comparison test fail for - to_system, rdata-$rdata_tmp expected-01 actual-$DE"
	inc ec
}
